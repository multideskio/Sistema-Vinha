...
class GenerateReportJob
{
    public function handle($data)
    {
        try {
            ...
            // Captura possíveis erros de consulta
            try {
                $transacoes = $transacoesQuery->findAll();
            } catch (\Exception $e) {
                log_message('error', 'Erro ao buscar transações: ' . $e->getMessage());
                return; // Continua sem travar o worker
            }
            ...
            foreach ($transacoes as $transacao) {
                try {
                    ...
                    continue; // Continua mesmo se um erro ocorrer em uma transação
                }
            }
            ...
            try {
                ...
                // Não interrompa o worker, continue a execução
            }
        } catch (\Exception $e) {
            log_message('error', 'Erro inesperado: ' . $e->getMessage());
            // Continua mesmo após um erro inesperado
        }
    }
}
