...
class UploadsLibraries
{
     ...
     */
    private function initializeS3(): void
    {
        try {
            ...
            if (!$credentials) {
                ...
                throw new RuntimeException('Credenciais do S3 não encontradas.');
            }
            ...
            foreach ($requiredKeys as $key) {
                if (empty($credentials[$key])) {
                    ...
                    throw new RuntimeException("Chave S3 faltante: $key");
                }
            }
    ...
    public function upload(UploadedFile $file, string $path): string
    {
        if (!$file->isValid() || $file->hasMoved()) {
            ...
            throw new RuntimeException("Arquivo inválido ou já movido.");
        }
        try {
        ...
        try {
            ...
            throw new RuntimeException('Falha ao fazer upload: ' . $localException->getMessage());
        }
    }
    ...
    private function uploadToLocal(UploadedFile $file, string $path): string
    {
        ...
        if (!is_dir($localDir)) {
            if (!mkdir($localDir, 0755, true) && !is_dir($localDir)) {
                ...
                throw new RuntimeException("Erro ao criar o diretório: $localDir. Verifique as permissões.");
            }
        ...
        if (!is_writable($localDir)) {
            ...
            throw new RuntimeException("Permissões insuficientes para escrever no diretório: $localDir");
        }
        ...
        if (!$file->move($localDir, basename($path))) {
            ...
            throw new RuntimeException('Erro ao mover o arquivo para o servidor local.');
        }
